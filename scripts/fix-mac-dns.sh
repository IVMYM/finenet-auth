...
echo ""
echo "若 system dig 仍是 198.18.*："
echo "  1) 菜单栏完全 Quit Clash/Surge（不是只关 System Proxy）"
echo "  2) 活动监视器结束残留进程"
echo "  3) 检查是否开了增强模式/TUN，关掉后重启 Mac"
echo "  4) 临时: networksetup -setdnsservers Wi-Fi 223.5.5.5 8.8.8.8"
echo "恢复正常后: finenet-auth authorize && finenet-auth diagnose"
